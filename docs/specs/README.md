...
- **使用者**：个人或小组，用于为自己的真实业务做品牌国际化设计。
- **核心场景**：分工作坊逐步练习 + 沿用同一份数据继续完成策划书。
- **不是**：课堂计时器、PPT 生成器。
...
| **Work 2** 目标市场选择（3 步） |  构建评估体系（候选/筛选/4×2 指标 + Hybrid 2 Delphi）/  评估候选市场（3×16 评分）/  矩阵 + 三档决策 | [work2/work2.md](work2/work2.md) |
| **Work 3** 价值主张与定位（6 步） |  场景细分 /  卖点挖掘 /  备选卖点 /  评分与矩阵 /  主张与定位 /  人格与 Slogan | [work3/work3.md](work3/work3.md) |
| **Work 4** 营销组合 |  路径与进入模式 /  产品 /  定价 /  渠道 /  传播促销 /  4P 汇总 | [work4/work4.md](work4/work4.md) |
| **Work 5** 策划书 | 封面/摘要/1 企业概况/2 环境(PEST+SWOT)/3 STP/4 4P4C/5 展望/参考文献 | [work5.md](work5.md) |

> 5 个 Work 规格已全部完成。数据流：Work 1（SBU/画像/价值体系/合成调研）→ Work 2（目标市场：decision.tier1 主战场 + tier2 观察，旧 schema 自动回退）→ Work 3（场景细分/价值主张/定位）→ Work 4（4P）→ Work 5（策划书汇总）。
> Work 2 采用 Hybrid 2 Delphi（论文 AI-Human Hybrids for Marketing Research, JM 2025）：1 call 招聘 + 5 call 深 persona + user 主持 + 可选 1 call 收敛；旧版数据载入时自动迁移（schemaVersion 1→2）。
...
- **手动模式**：显示可复制的提示词 + 结果粘贴区，使用者到外部 AI 工具操作后粘回。未配置 key 时强制此模式。
...
- **长任务**（work1 合成调研、work2 评估体系流水线与 Hybrid 2 Delphi、work3 流水线与逐 persona/卖点评分、work5 全文润色）：按**单元边界**暂停——work1 一个 (persona×重复) 为一单元，work2 流水线四单元与 persona 并行阶段，work3 每个流水线单元/评分单元，work5 每个章节。
  - 运行中按钮变 `暂停 · 42%`，点了在当前单元完成后暂停（变 `继续 · 5/12`），右侧 × 中止。
  - 后端非流式，一次 HTTP 请求进行中无法真暂停；暂停发生在单元之间。
  - 已完成单元的结果与完成键（如 `survey._doneKeys`、`_pipeDone`、delphi `phase`、work3 `_scoreDone`）写入 state 并 autosave，**跨刷新/跨步骤保留**。再次点运行自动跳过已完成单元，断点续跑、不重复花 token。
  - × 中止：abort 当前请求，已完成单元结果保留，状态置为 `paused`，按钮显示「继续」。
- 手动模式下没有后台进程，不显示暂停/中止键。
- **真流水线**（`API.aiPipeline`，work2 评估体系 / work3 多子动作步）：主按键按单元顺序执行；单单元失败自动降级为该单元手动箱，可继续/跳过后续单元；重跑语义——草稿类确认后整组替换，评分类只补未完成单元。

### AI 上下文机制（docs/lib/ai_context.js，2026-08-27）

所有工作坊的 AI 按钮统一走最小上下文包：

- **上下文节注册表**：每工作坊注册上下文节（来源 + 字段级截断上限），按钮声明 `needs`；相关节全给、无关节不给（精度优先，不 dump 整包 state）。
- **digest 缓存**：按「上游 state 指纹 + 选中节 + few-shot 选择」会话内缓存稳定前缀（≈1000 tokens 护栏，超限丢节并注明已省略）；稳定前缀在前 → 命中提供商自动 prompt 缓存。
- **「消息设置」折叠区**：每个 ai-box 默认收起——上下文节勾选（默认=按钮 needs）、示例（通用格式示例，不用演示案例）、消息预览 + 估算 token、重置为推荐。
- **双模式不变**：手动模式复制提示词 + 粘贴结果，消息预览复用同一 digest。
