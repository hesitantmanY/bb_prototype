...
Arora、Chakraborty 与 Nishimura 于 2025 年发表了《AI-Human Hybrids for Marketing Research: Leveraging Large Language Models (LLMs) as Collaborators》。作者们与一家财富 500 强食品企业合作，用 GPT-4 复现了该公司 2019 年的定性深访与定量概念测试（n=605，以原始人类研究为基准），实验分定性、定量两条线，都以原始人类研究为基准逐项对照。文章实验过程如下：
...
- 二，LLM 可以低成本扮演合成受访者、访谈主持与分析师：设定样本特征、生成 persona、按提纲追问（回答质量低于阈值自动追问）、把长文本提炼为主题与摘要。本平台的合成调研实现其中受访者与分析师两个角色（画像作答 + 校验重试 + 开放题主题提炼，未实现自动追问），Delphi 专家面板借用的是“LLM 建议样本”的招聘思路；
