...
评估做四层：NLP 指标（可读性、信息密度、LDA 主题一致性）、BERT 句向量降维后与人类数据的语义距离、Prolific 上 250 名评估者的盲评、专家评委对摘要的盲选。混合 3 在前两层指标上表现最好，后续评估以它的数据为准；人工划重点平均 35 句、LLM 只挑 19 句，但两组所圈句子的余弦相似度达 .78。摘要环节由 10 名五年以上经验的分析师与 LLM 分别完成“划重点—聚类主题—写摘要”（每题审 23 名受访者的回答约需 40–45 分钟），5 题 × 2 数据来源 × 2 分析者 = 20 份摘要，交 5 名十年以上经验的评委逐题盲选。结果显示：人类评估者认为 LLM 回答的深度与洞察性分别高出约 0.68 与 0.50 分（五点量表）；LLM 分析师主题召回率达 77%–96%，还能发现人类遗漏的新主题；专家评选最佳摘要时没有任何一位选择纯人类或纯 LLM 的版本。
...
复现结果如下：零样本为 .66 / .41 / .40——方向与效价抓得住，但回答挤在 4–5 分、态度题间相关几乎为零（人类数据中“最健康配料”与“最高品质”相关 .69）；few-shot 把异质性与相关差降到 .29 / .27，再加 RAG 后相关差降到 .11、最贴近人类的相关结构；但三者的均值偏差都在 .7 左右，注入上下文并未改善。论文由此得出三点核心结论：人机混合优于任何单边；LLM 可以低成本扮演合成受访者、访谈主持与分析师；零样本合成回答能抓住方向与效价，但异质性与内部一致性不足，需要用 few-shot 与 RAG 注入上下文改善。

### 本项目如何模仿设计

本项目没有复现论文的评估实验，而是把论文验证可行的机制翻译成工作坊功能，逐条对应如下：
