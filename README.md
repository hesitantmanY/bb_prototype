...
Arora、Chakraborty 与 Nishimura 于 2025 年发表了《AI-Human Hybrids for Marketing Research: Leveraging Large Language Models (LLMs) as Collaborators》。作者们与一家财富 500 强食品企业合作，用 GPT-4 复现了该公司 2019 年的定性深访与定量概念测试（n=605，以原始人类研究为基准），文章实验过程如下。

实验分定性、定量两条线，都以原始人类研究为基准逐项对照。定性侧（Study 1）复现的是 2019 年围绕 Friendsgiving（朋友间庆祝的感恩节）的线上深访：讨论提纲保持不变，按“谁选样本 / 谁当受访者 / 谁来追问”组合出四种人机混合条件：
...
混合 4 是二者的叠加。评估做四层：NLP 指标（SMOG / Gunning fog / Flesch–Kincaid 可读性、信息密度、LDA 主题一致性）、BERT 句向量加 UMAP 降维后与人类数据的语义距离、Prolific 上 250 名评估者的盲评（每人 4 段问答、LLM 与人类各 2 段，共 1,000 次评估，混合效应模型控制答案长度与评估者特征；深度与洞察性显著更高，清晰度与相关性无差异）、专家评委对摘要的盲选。
混合 3 在前两层指标上表现最好，后续评估均以它的数据为准；人工划重点平均 35 句、LLM 只挑 19 句，但两组所圈句子的余弦相似度达 .78。摘要环节由 10 名五年以上经验的分析师与 LLM 分别完成“划重点—聚类主题—写摘要”（每题审 23 名受访者的回答约需 40–45 分钟），5 题 × 2 数据来源 × 2 分析者 = 20 份摘要，交 5 名十年以上经验的评委逐题盲选。

定量侧（Study 2）复现的是 2019 年冷藏狗粮概念测试（605 名狗主）：概念为可重封袋 vs 可切片管两种包装 × fresh 与 human grade 两种食材，测量概念偏好、购买可能性 / 喜爱 / 独特性、7 项态度李克特题与 9 档购买频次。作者用 GPT-4（temperature=1；改变温度与随机种子结果稳健）按原样本人口特征逐人生成 605 个合成受访者（304 人评可重封袋、301 人评可切片管），比较三种上下文注入：
-LLM1 零样本（仅 persona + 题目）；
-LLM2 few-shot（把同一受访者已答的前几题问答对拼进系统提示）；
-LLM3 few-shot + RAG（把公司此前一项高档宠物食品定性研究中 16 名受访者的转录做成向量库，按题检索注入）。以与人类数据的均值偏差、异质性（标准差）差、两两相关差的绝对值衡量（越低越好）。
复现结果如下：零样本为 .66 / .41 / .40——方向与效价抓得住，但回答挤在 4–5 分、态度题间相关几乎为零（人类数据中“最健康配料”与“最高品质”相关 .69）；few-shot 把异质性与相关差降到 .29 / .27，再加 RAG 后相关差降到 .11、最贴近人类的相关结构；但三者的均值偏差都在 .7 左右，注入上下文并未改善。

此实验得到三个与本平台直接相关的结论：
一，人机混合优于任何单边。定性侧，人类评估者认为 LLM 生成的回答在深度与洞察性两个维度分别高出约 0.68 与 0.50 分（五点量表）；LLM 担任分析师时主题召回率达 77% 至 96%，还能发现人类遗漏的新主题；专家评委评选最佳摘要时，没有任何一位选择纯人类或纯 LLM 的版本；
二，LLM 可以低成本扮演合成受访者、访谈主持与分析师：设定样本特征、生成 persona、按提纲追问（回答质量低于阈值自动追问）、把长文本提炼为主题与摘要。本平台的合成调研与 Delphi 专家面板正是这一用法；
三，定量侧零样本 LLM 能抓住答案方向与效价，但异质性与内部一致性不足，需要用 few-shot 与 RAG 注入上下文改善。
