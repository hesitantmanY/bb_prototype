...
实验分定性、定量两条线，都以原始人类研究为基准逐项对照。定性侧（Study 1）复现的是 2019 年围绕 Friendsgiving（朋友间庆祝的感恩节）的线上深访：讨论提纲保持不变，按“谁选样本 / 谁当受访者 / 谁来追问”组合出四种人机混合条件——混合 1 依原受访者画像（性别、年龄、族裔、主人或宾客角色）生成合成受访者；混合 2 让 LLM 先建议该研究该访谈谁（补上了原研究漏掉的饮食限制者、留学生与外籍人士、LGBTQ+ 群体、开发 Friendsgiving 菜单的厨师），再据此生成 10 个 persona；混合 3 让 LLM 既当受访者又当主持，对每轮问答按清晰度 / 相关性 / 深度 / 洞察性四维实时打分（0–100），低于 80 分自动追问、再打分，达标才进下一题；混合 4 是二者的叠加。评估做四层：NLP 指标（SMOG / Gunning fog / Flesch–Kincaid 可读性、信息密度、LDA 主题一致性）、BERT 句向量加 UMAP 降维后与人类数据的语义距离、Prolific 上 250 名评估者的盲评（每人 4 段问答、LLM 与人类各 2 段，共 1,000 次评估，混合效应模型控制答案长度与评估者特征；深度与洞察性显著更高，清晰度与相关性无差异）、专家评委对摘要的盲选。混合 3 在前两层指标上表现最好，后续评估均以它的数据为准；人工划重点平均 35 句、LLM 只挑 19 句，但两组所圈句子的余弦相似度达 .78。摘要环节由 10 名五年以上经验的分析师与 LLM 分别完成“划重点—聚类主题—写摘要”（每题审 23 名受访者的回答约需 40–45 分钟），5 题 × 2 数据来源 × 2 分析者 = 20 份摘要，交 5 名十年以上经验的评委逐题盲选。

定量侧（Study 2）复现的是 2019 年冷藏狗粮概念测试（605 名狗主）：概念为可重封袋 vs 可切片管两种包装 × fresh 与 human grade 两种食材，测量概念偏好、购买可能性 / 喜爱 / 独特性、7 项态度李克特题与 9 档购买频次。作者用 GPT-4（temperature=1；改变温度与随机种子结果稳健）按原样本人口特征逐人生成 605 个合成受访者（304 人评可重封袋、301 人评可切片管），比较三种上下文注入：LLM1 零样本（仅 persona + 题目）、LLM2 few-shot（把同一受访者已答的前几题问答对拼进系统提示）、LLM3 few-shot + RAG（把公司此前一项高档宠物食品定性研究中 16 名受访者的转录做成向量库，按题检索注入）。以与人类数据的均值偏差、异质性（标准差）差、两两相关差的绝对值衡量（越低越好）：零样本为 .66 / .41 / .40——方向与效价抓得住，但回答挤在 4–5 分、态度题间相关几乎为零（人类数据中“最健康配料”与“最高品质”相关 .69）；few-shot 把异质性与相关差降到 .29 / .27，再加 RAG 后相关差降到 .11、最贴近人类的相关结构；但三者的均值偏差都在 .7 左右，注入上下文并未改善。
