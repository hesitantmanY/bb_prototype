...
Arora、Chakraborty 与 Nishimura 于 2025 年在《Journal of Marketing》发表《AI-Human Hybrids for Marketing Research: Leveraging Large Language Models (LLMs) as Collaborators》。论文用一家财富 500 强食品企业的两个真实研究验证：LLM 能低成本扮演合成受访者、访谈主持与分析师，且人机混合优于纯人类或纯 LLM。本项目不照搬论文实验，而是把论文验证可行的机制逐条落成工作坊里的具体功能。
...
定性侧（Study 1）复现 2019 年 Friendsgiving 线上深访，按“谁选样本 / 谁当受访者 / 谁来追问”组合出四种人机混合条件（依画像生成受访者、先招聘再生成 persona、LLM 受访者兼主持自动追问、二者叠加）。四层评估（NLP 指标、句向量语义距离、250 人盲评、专家评委盲选）显示：人类评估者认为 LLM 回答的深度与洞察性分别高出约 0.68 与 0.50 分（五点量表）；LLM 分析师主题召回率达 77%–96%；专家评选最佳摘要时没有任何一位选择纯人类或纯 LLM 版本。

定量侧（Study 2）按原样本人口特征逐人生成 605 个合成受访者，比较零样本、few-shot、few-shot + RAG 三种上下文注入。结果零样本能抓住方向与效价，但回答挤在 4–5 分、态度题间相关几乎为零；few-shot 把异质性与相关差降到 .29 / .27，再加 RAG 后相关差降到 .11，但三者均值偏差仍约 .7——注入上下文改善结构，不改善均值偏差。

明确不模仿的部分：未复现论文的评估实验（无人类盲评、NLP 指标与句向量对照）；未实现混合 3 的自动追问；Delphi 采用单轮“招聘 → 画像赋权 → 均值归一化”，不是多轮往返。模拟语料始终标注「模拟」，不能替代真实问卷与评论。
